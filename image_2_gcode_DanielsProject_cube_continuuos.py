...
def image2gcode_spiral_cube(image_list, toggle_ON_list, toggle_OFF_list, visualize_ON, fil_width, z_height, z_var, wall_thickness,offset_ON, offset_OFF):
    ...
    if height % 2 != 0:  # if the height is an odd number, starts the the print on the inner walls
        p_list_odd = p_list_A
    ...
    else: # if the even, starts the print on the outer walls
        p_list_odd = p_list_B
    ...
    ## This section reverses the pixels ever other row
    reversed_image_list = img_list
    for i in range(len(img_list)):
        for j in range(len(img_list[i])):
            ...
            if layer_flag == reverse_pixels:
                current_image_row = np.flip(img_list[i][j])
            else:
                current_image_row = img_list[i][j]

            reversed_image_list[i][j] = current_image_row

    ## This section applies offsets to the image pixels
    img_list_offsets = reversed_image_list
    offset = offset_ON
    for i in range(len(reversed_image_list)):
        #current_image = reversed_image_list[i]
        for j in range(len(current_image)):
            current_image_row = current_image[j]
            ...
            if i+1 != ending_image:
                current_image_row = current_image_row[offset:] # removes first n pixels of current image
                next_image_row = reversed_image_list[i + shift][j][0:offset] # first n pixels of next image
                offset_pixel_row = np.append(current_image_row, next_image_row) # removes n pixels from beginning of current image, appends first n pixels from next image

                img_list_offsets[i][j] = offset_pixel_row

            elif ending_image == 4 and j+1 < len(current_image):
                current_image_row = current_image_row[offset:]  # removes first n pixels of current image
                next_image_row = reversed_image_list[i][j+1][0:offset]  # first n pixels of next image
                offset_pixel_row = np.append(current_image_row,next_image_row)  # removes n pixels from beginning of current image, appends first n pixels from next image
                ...
                for i in range(offset):
                    offset_pixel_row = np.append(current_image_row, white)  # addes white pixels
...
if __name__ == '__main__':
    ...
    print('--------------------')
    offset = 5
    G_list_offset = G_list
    var_list_offset = var_list

    dist_list_offset = dist_list
    command_list_offset = command_list
...
''' Take 1 of offset'''
    ...
    #     offset = 2
    #     G_list_offset.append(G_list[i])
    #     var_list_offset.append(var_list[i])
    #     dist_list_offset.append(dist_list[i])
    #     command_list_offset.append(command_list[i])
    #
    ...
    #                 offset_dist = -offset_dist
    #
    #             dist_list_offset[i] = offset_dist
    #
    #             if var_list[i] == var_list[i+1]:
    #                 if dist_list[i+1] < 0:
    #                     offset = -offset
    #
    #                 dist_list_offset[i+1] = dist_list[i+1] + offset
    #
    #         else:
    #             count = 0
    #             while offset_dist < 0:
    ...
    #                 offset_dist = - offset_dist
    #
    #             dist_list_offset[i-count] = offset_dist
    #             command_list_offset[i-count] = command_list[i]
    #
    #             dist_list_offset[i] = 0
    #             command_list_offset[i] = ''
    #
    #             dist_list_offset.insert(i - count + 1), offset_new)
    #             G_list_offset.insert(i - count + 1), G_list[i-count])
    #             command_list_offset.insert(i - count + 1, '')
    ...
    #         if var_list[i] == 'Z':
    #             f.write('\n------ new layer')
    #         f.write('\n' + str(G_list_offset[i]) + ' ' + str(var_list_offset[i]) + str(dist_list_offset[i]))
    #         f.write('\n' + command_list_offset[i])
